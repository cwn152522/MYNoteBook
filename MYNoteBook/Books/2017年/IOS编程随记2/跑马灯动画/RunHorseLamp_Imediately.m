...
- (void)configRunHorseLamp{
    self.lampView = [[RunHorseLampView alloc] initWithFrame:CGRectMake(0, 64, CGRectGetWidth([UIScreen mainScreen].bounds), 27) text:nil options:([RunHorseLampViewOptions optionsWithDuration:7.0f textColor:nil textFont:[UIFont systemFontOfSize:13] backgroundColor:nil])];
    [self.view addSubview:self.lampView];

    self.lampView.stopType = RunHorseLampStopImediately;

//    [self requestTips];//网络请求
